...
#elif HSI_SYSCLK_FREQ_48MHz
  const uint32_t SystemFrequency         = HSI_SYSCLK_FREQ_48MHz;      /*!< System Clock Frequency (Core Clock) */
  const uint32_t SystemFrequency_SysClk  = HSI_SYSCLK_FREQ_48MHz;      /*!< System clock                        */
  const uint32_t SystemFrequency_AHBClk  = HSI_SYSCLK_FREQ_48MHz;      /*!< AHB System bus speed                */
  const uint32_t SystemFrequency_APB1Clk = (HSI_SYSCLK_FREQ_48MHz/2);  /*!< APB Peripheral bus 1 (low)  speed   */
  const uint32_t SystemFrequency_APB2Clk = HSI_SYSCLK_FREQ_48MHz;      /*!< APB Peripheral bus 2 (high) speed   */
...
void SetHsiClock48( void )
{
    ...
    RCC_PCLK2Config( RCC_HCLK_Div1 ); // Configures the High Speed APB clock (PCLK2). PCLK2=HCLK/1=64MHz
    RCC_PCLK1Config( RCC_HCLK_Div2 ); // Configures the Low Speed APB clock (PCLK1).  PCLK1=HCLK/2=32MHz

    RCC_PLLConfig( RCC_PLLSource_HSI_Div2, RCC_PLLMul_12 ); // Configures the PLL clock source and multiplication factor. 8MHz/2 * 4 = 16MHz
